...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
        ...
        @Query("SELECT oi FROM OrderItem oi " +
                        ...
                        "AND oi.type = :type " +
                        "ORDER BY oi.createdAt DESC")
        List<OrderItem> findByCustomerIdAndType(@Param("customerId") Long customerId,
                        @Param("type") OrderItemType type);
...
Page<OrderItem> findBySellerIdWithDetails(@Param("sellerId") Long sellerId, Pageable pageable);

        /**
         * Find all order items for a seller with order and product details
         * Used for seller-centric order item management
         * 
         * @param sellerId - The seller ID
         * @return List of OrderItems belonging to that seller's products
         */
        @Query("SELECT oi FROM OrderItem oi " +
                        ...
                        "WHERE p.seller.id = :sellerId " +
                        "ORDER BY oi.createdAt DESC")
        List<OrderItem> findBySellerIdWithOrderAndProductDetails(@Param("sellerId") Long sellerId);
}